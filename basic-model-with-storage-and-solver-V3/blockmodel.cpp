...
bool BlockModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (data(index, role) != value) {
        ...
        case BlockYPositionRole:
            ...
            break;
        case EquationRole:
            if(blockItem->equation() != value.toString()){
                blockItem->setEquation(value.toString());
                blockItem->setEquationObject(value.toString());
            }
            break;
        }
...
void BlockModel::setBlockDataSource(BlockDataSource *dataSource)
{
    ...
    //Connect the datasource signals for addeing and removing rows from the model to
    //QAbstractListModel
