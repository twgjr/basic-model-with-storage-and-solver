...
BlockDataSource::BlockDataSource(QObject *parent) : QObject(parent),
    /*
        ...
        m_param(m_context)*/
    m_recurseLimit(1000),
    m_equationSolverObj(&m_context,m_recurseLimit)
{
...
bool BlockDataSource::loadBlockItems(QVariant loadLocation)
{
    ...
    if (json.contains("Blocks") && json["Blocks"].isArray()) {
        ...
        for ( int i = 0; i < blockArray.size() ; i++){
            ...
            BlockItem * blockItem = new BlockItem(&m_context,m_recurseLimit,this);
...
{
    m_equationSolverObj.resetSolver();
    m_equationSolverObj.clearEquationList();

    //append all equations
    for(int i = 0; i < m_BlockItems.size(); i++)
    {
        m_equationSolverObj.appendEquation();
        m_equationSolverObj.equationList().at(m_equationSolverObj.equationList().size()-1)->setEquationExpression(m_BlockItems.at(i)->equationObject()->equationExpression());
    }

    //solve
    m_equationSolverObj.solveEquations();

    //set solution to return to app
}

//EquationSolver BlockDataSource::equationSolverObj() const
//{
//    return m_equationSolverObj;
//}
